    ...
    client.commands['upgrade'] = {
        ...
        run(message) {
            client.load_user_data(message.author.id, res => {
                ...
                if(args[0] == 'scan'){
                    let price = res.ship.scanner_strength * client.settings.game.scan_upgrade_cost
                    if(res.credits - price < 0) return client.send_error(message, `You aren\'t able to afford this upgrade. This will cost you \`${price}\` credits.`)
                    if(res.ship.scanner_strength + client.settings.game.scan_upgrade_amount > res.ship.max_scanner_strength) return client.send_error(message, `Your ship type \`${res.ship.type}\` cant fit any more scan upgrades.`)
                ...
                if(args[0] == 'defense'){
                    ...
                    collector.on('collect', message => {
                        if (message.content.toLowerCase() == `yes`) {
                            message.reply('You have successfully upgraded your ship.')
                            res.credits -= price
                            res.ship.def += client.settings.game.def_upgrade_amount
                            client.write_user_data(message.author.id, res)
                            client.cooldowns.collector.splice(client.cooldowns.collector.indexOf(message.author.id), 1)
                            collector.stop()
                        }
